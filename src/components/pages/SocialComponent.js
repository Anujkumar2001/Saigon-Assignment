...
export const SocialComponent = ({ title, description, buttonText }) => {
  ...
  return (
        <div className="tl-4-social" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      <div className="tl-4-social-txt">
        <h2 className="tl-4-social-title">{title}</h2>
        <div className={"tl-4-social-hidden-content " + (showHiddenContent ? 'show' : 'hidden') } style={{ display: showHiddenContent ? 'block' : 'none' }}>
